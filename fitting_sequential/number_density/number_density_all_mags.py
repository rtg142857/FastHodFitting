...
if __name__ == "__main__":
    ...
    params_final = np.genfromtxt("../hod_params/params/fit_params.txt")
    for i in range(9):
        log_Mmin, sigma, log_M0, log_M1, alpha = np.genfromtxt("../hod_params/params/fit_params.txt")[i,1:]
        ...
        log_Mmin_new = match_target_density(log_Mmin, sigma, log_M0, log_M1, alpha, log_n_targs[i], log_mass_halo, log_n_halo)
    
        print("Original log_Mmin:\t", log_Mmin)
        print("Match number density:\t", log_Mmin_new)
        print("Ratio:\t\t\t", 10**log_Mmin_new/10**log_Mmin)
        params_final[i,1] = log_Mmin_new

    np.savetxt("params_fixed_number_density.txt",params_final)
