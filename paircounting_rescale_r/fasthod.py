...
def read_hdf5(path):
    ...
    x = position[:,0] * rescale_r
    y = position[:,1]* rescale_r
    z = position[:,2]* rescale_r
    Mvir = snap["/mass"][:]
...
def read_hdf5_more_files(path):
    ...
    for i in range(1,34):
        ...
        x_temp = position_temp[:,0]* rescale_r
        y_temp = position_temp[:,1]* rescale_r
        z_temp = position_temp[:,2]* rescale_r
        Mvir_temp = snap["/mass"][:]
...
def read_ascii(path):
    ...
    y = y[mask1]* rescale_r
    z = z[mask1]* rescale_r
    vx = vx[mask1]* rescale_r
    vy = vy[mask1]* rescale_r
    vz = vz[mask1]* rescale_r
    Vrms = Vrms[mask1]* rescale_r
